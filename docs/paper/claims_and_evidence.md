...
| C23 | **Heavier simulated augmentation (rotation, gamma, bias field, elastic) does not close the out-of-distribution gap** — pooled SSA+PED `dice_TC` +0.0117, CI [−0.0006, +0.0245], p_holm 0.936, and 12 of 12 pre-registered comparisons inconclusive | Note 48; pre-registered `preregistration_augmentation.md` before the run; one Holm family of 12 across both cohorts and both metric conventions | **Moderate negative, explicitly underpowered.** The power statement was fixed in advance: at n=159 with one seed an effect of the observed size is below resolution. Read as "not demonstrated", never as "augmentation cannot help". In distribution nothing moved (largest |Δ| 0.0007), so it is not a trade-off either |
