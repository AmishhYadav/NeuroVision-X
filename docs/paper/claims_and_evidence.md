...
|---|---|---|---|
| C1 | The proposed dual-encoder architecture improves enhancing-tumour segmentation over a matched U-Net baseline | ET Dice +0.0267, CI [0.0166, 0.0393], p_holm 1.4e-21, n=189 paired. **Seed-to-seed noise floor measured (D1, note 49):** test `dice_ET` seed43−seed42 = +0.0021, CI [−0.0018, +0.0068] — ~13x smaller than the headline margin | **Strong.** Large effect, tiny p, matched training recipe, and now checked against the noise floor rather than asserted single-seed |
...
| C20 | **End-to-end, the pipeline returns a usable mask (WT and TC Dice ≥ 0.7) for 85.2% of in-distribution studies [79.9, 90.0], 78.3% of SSA [66.7, 88.3] and 24.2% of PED [16.2, 33.3]** — measured through every stage with the deployed frozen gate, nothing refit | Note 47; protocol fixed first (`error_budget_protocol.md`). Silent-failure rate (accepted but unusable) 4.2% / 18.3% / 49.5% | **Strong, descriptive.** The Phase G deliverable. Always print the silent-failure column beside it |
| C21 | **The deployed refusal gate has decent precision and poor recall, and is blind to cohort-level shift**: in distribution it refuses 16 usable studies to catch 4 unusable ones; on SSA it refuses 2 of 60 while passing 11 of 12 unusable masks; on PED 23 of 26 refusals are correct (precision 0.88) but recall on unusable is 0.32, and no operating point on the coverage curve gets P(usable \| accepted) above 0.5 | Note 47, G3/G5 tables | **Strong, descriptive.** This is the quantified version of "refuses inputs it cannot handle" and it must be written this way, not as a safety property |
| C22 | Gate acceptance does not restore the conformal guarantee where it breaks: PED TC realised risk 0.651 on all cases, 0.573 on accepted ones, at nominal 0.10 | Note 47 | **Strong.** Consistent with C14; the gate filters the worst cases but cannot repair a broken exchangeability assumption |
| C23 | **Heavier simulated augmentation (rotation, gamma, bias field, elastic) does not close the out-of-distribution gap** — pooled SSA+PED `dice_TC` +0.0117, CI [−0.0006, +0.0245], p_holm 0.936, and 12 of 12 pre-registered comparisons inconclusive. **Independently confirmed as noise (D1, note 49):** a descriptive pooled SSA+PED `dice_TC` seed43−seed42 read gives +0.0111, CI [−0.0056, +0.0276] — essentially the same magnitude as D0's own +0.0117, so the entire observed margin is explainable by seed noise alone | Note 48; pre-registered `preregistration_augmentation.md` before the run; one Holm family of 12 across both cohorts and both metric conventions | **Moderate negative, explicitly underpowered — and now doubly confirmed.** The power statement was fixed in advance: at n=159 with one seed an effect of the observed size is below resolution. Read as "not demonstrated", never as "augmentation cannot help". In distribution nothing moved (largest |Δ| 0.0007), so it is not a trade-off either |
| C24 | **Counterfactual: a new site can restore the conformal bound by recalibrating on its own labelled cases — ~10–40 of them depending on α — for whole tumour on SSA and PED and tumour core on SSA; paediatric tumour core cannot be restored at any k.** Control (BraTS test, k = half) restored everywhere | Note 52; `preregistration_conformal.md` Amendment 1, fixed before the run; 1000 seeded splits per cell, both models | **Strong, but COUNTERFACTUAL — never external validation, never "this cohort's coverage".** Always print the floor beside the k: below it, feasible splits are biased optimistic (SSA · TC, α 0.05, k 30: 0.054). Per-split violation up to ~30% is expected under a marginal guarantee and must be explained where quoted |
...
| "The confidence head provides useful uncertainty" | **Measured 2026-08-24 and refuted.** Worse than free entropy on all three regions, and on WT it is at chance (0.4766) with entropy winning on all 189 cases (rank-biserial −1.00). It is NOT dead everywhere — ET 0.855 and TC 0.871 are well above chance, and its residualised AUROC on TC is 0.662, so it does see something entropy does not. But it never wins. Note 44 |
...
| "The refusal gate makes the pipeline safe to deploy under distribution shift" / "the gate refuses inputs it cannot handle" (as a property) | **Refuted by note 47.** On SSA the gate is a no-op (2 refusals in 60, 11 of 12 unusable masks passed); on PED half the cohort is accepted with an unusable mask. It catches *individually* bad masks with precision ~0.9 and recall ~0.3 in the cell where it works at all. Write the numbers, not the property |
| "conformal_band is a useful refusal signal" | Note 47: every `conformal_band` refusal in distribution was an over-refusal (12 of 12); 4 of 26 refusals on PED. Report it as a signal that was enabled and measured, not as one that works |
| "Heavy augmentation helps the paediatric cohort" | The per-cohort split behind note 48's pooled null (PED +0.0329 ET / +0.0284 TC, SSA −0.0159 TC) was **not pre-registered**, the declared family is pooled, and per-cohort n is smaller than the pooled n that already failed to resolve. It is a direction to test in D2/D3, not a result |
...
1. **Single seed for most numbers.** D1 (note 49) measured ONE seed-to-seed difference for `neurovision` (test/SSA/PED), giving a noise floor of roughly ±0.002–0.01 Dice depending on cohort/region — but two seeds is a floor, not a distribution: there is no seed-level variance estimate, so a margin below that floor still is not claimable, and the P2 null remains "no detectable difference at n=189", not equivalence. A TOST with a pre-set margin would still need a proper variance estimate (≥3 seeds, not yet run). Every OTHER model in this project (baseline, ablations, capacity control) remains single-seed.
