...
| D1 | Second seed of `neurovision` (seed 43), 3 chained T4 sessions | `[x]` **training COMPLETE 2026-09-23** on the current augmentation, as D0's NULL prescribes (`preregistration_multiseed.md`). s1: epoch 33 (best 29). s2: epoch 70 (best 69). s3 (`neurovision-d1-seed43-s3`): epoch 79 (best 79), `NVX_HEALTH: OK`, `nonfinite=[]`, loss 0.4545, 7.66 GiB peak VRAM reserved. All three on one W&B run `iriee13d`, `GIT_REF=9c770ce`. **Not yet done:** `evaluate.py` on test/SSA/PED (`save_logits=true`) + `replay_logits.py` + the 12-comparison seed-to-seed paired family — CPU, local, not started | Three sessions complete at epoch 79, then `evaluate.py` on test/SSA/PED and the 12-comparison seed-to-seed family |
