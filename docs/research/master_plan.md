...
| D1 | Second seed of `neurovision` (seed 43), 3 chained T4 sessions | `[~]` **in progress 2026-09-23** on the current augmentation, as D0's NULL prescribes (`preregistration_multiseed.md`). s1 COMPLETE: epoch 33 (best_epoch 29). s2 (`neurovision-d1-seed43-s2`) COMPLETE: epoch 70 (best_epoch 69), `NVX_HEALTH: OK`, `nonfinite=[]`, loss 0.4521, 6.17 GiB peak VRAM. Both on W&B run `iriee13d`, `GIT_REF=9c770ce`. s3 (`neurovision-d1-seed43-s3`) pushed and RUNNING, resuming from s2 at epoch 70, target epoch 79 (final session) | Three sessions complete at epoch 79, then `evaluate.py` on test/SSA/PED and the 12-comparison seed-to-seed family |
