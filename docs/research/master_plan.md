...
| D1 | Second seed of `neurovision` (seed 43), 3 chained T4 sessions | `[~]` **in progress 2026-09-21** on the current augmentation, as D0's NULL prescribes (`preregistration_multiseed.md`). s1 (`neurovision-d1-seed43-s1`) COMPLETE: epoch 33 (best_epoch 29), `NVX_HEALTH: OK`, `nonfinite=[]`, 1002 s/epoch, 6.16 GiB peak VRAM, W&B run `iriee13d`, `GIT_REF=9c770ce`. s2 (`neurovision-d1-seed43-s2`) pushed and QUEUED, resuming from s1 at epoch 33 | Three sessions complete at epoch 79, then `evaluate.py` on test/SSA/PED and the 12-comparison seed-to-seed family |
