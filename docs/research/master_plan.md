...
| E7 | UI — bounded mask, QC estimate, refusal banner | `[~]` 3D digital-twin viewer + landing page built 2026-08-26 (uncommitted work found and finished); refusal-banner/QC-estimate UI for the NEW clinical pipeline still open | `app/frontend/`; extend the existing E2E harness rather than replacing it |
| — | Wire E1–E5 into `app/backend/` | `[x]` **`clinical_jobs.py` built 2026-08-26** — E1→E3→E2→E3→segment(neurovision, pinned)→QC/conformal signals→E5, `"refused"` a distinct job state from `"failed"`; 17 tests, full suite green | E6 deliberately excluded (see E6 row). `api.py` HTTP routes and frontend wiring are the remaining, still-queued pieces to make this reachable from the browser |
