...
| P1.1 | **Local recalibration** (the pre-registered Mondrian arm + a k-sweep 5/10/15/20/30, 1000 seeded splits, α 0.05/0.10/0.20, WT/TC, both models, SSA/PED, test as control). Amendment committed first | `[x]` 2026-09-26 **note 52, C24**: all 4 predictions held; control restored; SSA WT/TC + PED WT restorable at ~10–40 local cases; **PED·TC never** — driver `ff5b7dd` | A numbered note + claim C24, labelled **counterfactual** |
| P1.2 | T0.4 one-case pilot (RSNA `train/00000`) → real-DICOM validation with ground truth, ≤40 RSNA cases whose BraTS ID is in **test**. Protocol doc first | `[~]` protocol + frozen sample `44bffa9` (90 eligible, 40 drawn); scorer `71c0e24`; driver `d7eac73`; 41 studies downloaded (925 MB, `data/rsna/studies`); **real run next** (~4 CPU-h) | A numbered note; a real-DICOM stage row in the error budget |
| P1.3 | Intended-use signal ("adult glioma only") from DICOM `PatientAge` | `[~]` author: missing age → CAUTION (2026-09-26); ingest reads age `P1.3a`; gatekeeper signal + error-budget re-score next | A numbered note, reported as **scoping, never detection** |
...
| P2.1 | `training.init_from` — weights-only, fresh optimizer | `[x]` 2026-09-26 `aa29714` | CPU tests: loads weights; `last.pt` resume beats `init_from` |
| P2.2 | Frozen cross-fit splits `configs/data/splits_{ssa,ped}_cf{0,1}.yaml` (seeded generator) | `[x]` 2026-09-26 `adc69f1` — SSA 25/5/30 (120 ep), PED 45/5/49 (67 ep), 44/5/50 (68 ep) | Committed before any training; old SSA/PED split files untouched |
...
|---|---|---|
| D | D0 heavier-augmentation ablation → D1 multi-seed → D2 pooled multi-cohort → D3 fine-tune-on-SSA | **D0 `[x]` RESOLVED 2026-09-20 — verdict NULL, recipe unchanged** (23.7 GPU-h, 12/12 inconclusive, primary pooled `dice_TC` +0.0117 CI [−0.0006, +0.0245]; note 48, `preregistration_augmentation.md` §Result). D1 therefore trains seed 43 on the **current** augmentation. D2/D3 unchanged. D1 also unblocks B3 |
