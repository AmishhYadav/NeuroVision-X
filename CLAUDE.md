...
- **Working:**
  ...
  - **317 tests, CPU, ~5s.** Bare `pytest` works from repo root (`pythonpath = ["src"]`)
...
- **Next:** upload `outputs/kaggle_upload` to Kaggle (`amishyadav123/neurovision-brats-prep`, PRIVATE), push the repo, then the Kaggle baseline run via `notebooks/kaggle_train.ipynb`
...
- **The two encoder branches do not downsample by the same rule, and only agree because the patch is 96³.** `CNNEncoder` (`src/neurovision/models/encoders/cnn.py`) downsamples with padded stride-2 convolutions, which give `ceil(D/2)` per level — odd sizes round *up*, so 25 → 13. SwinUNETR downsamples by non-overlapping **patch merging**, which consumes 2 voxels per axis per merge and does not follow the ceil convention. At 96³ every level divides evenly and the two branches produce identical spatial shapes at strides 2/4/8, so the CNN's levels 1/2/3 line up with `hidden_states_out[0:3]` exactly. At an odd or non-power-of-two patch size they can differ by a voxel, and the fusion module would then either raise on `torch.cat` or, worse, silently broadcast. Consequence: **96³ is now load-bearing for a second reason beyond the VRAM budget** (hard constraint 6), and the fusion module needs an explicit shape-agreement test between the two pyramids on the day it is written — do not assume alignment survives a patch-size ablation.
