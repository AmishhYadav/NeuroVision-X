...
- **Not built yet, from the architecture in "What this project is":** MC-dropout uncertainty, calibration/ECE analysis, and explainability (Integrated Gradients). All three heads now exist and train; `forward_with_gates` (fusion gate-map figure) and `forward_multitask` (always returns a `MultiTaskOutput`, in either mode — the hook for the uncertainty/calibration code) are in place but nothing consumes either yet
